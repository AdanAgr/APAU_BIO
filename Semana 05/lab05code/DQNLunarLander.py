...
class Agent():
    ...
    def train_model(self, epochs=1000, threshold=200):
        ...
        for epoch in range(epochs):
            ...
            if mean_score >= threshold and len(scores) == 100:
                ...
                torch.save(self.model.state_dict(), 'dqn_lunarlander.pth')
                plt.ioff()
                return self.avg_scores

            if epoch % 100 == 0:
                print(f'[Episode {epoch}] - Mean reward over last 100 episodes: {mean_score}.')
    ...
    def load_weights_and_visualize(self):
        self.model.load_state_dict(torch.load('dqn_lunarlander.pth'))
        self.model.eval()
